...
def start_server(config):
    ...
    print('start server rpc:', port_rpc)
    ...
    def dispatch(port_rpc):
        with SimpleXMLRPCServer(('localhost', port_rpc),
...
def start_scene(config, nb_actions):
    ...
    results = {}
    sdict = str(config)
    # subprocess.Popen([config['python_version'], path+"common/rpc_client.py", sdict, str(nb_actions), str(port_rpc)])
